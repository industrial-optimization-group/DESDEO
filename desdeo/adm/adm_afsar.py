...
from desdeo.tools.non_dominated_sorting import fast_non_dominated_sort as nds
...
class ADMAfsar:
    def __init__(
        ...
        self.num_dims = num_objectives
        # Initial random reference point
        self.preference = [
            np.random.uniform(min_val, max_val)
            for min_val, max_val in zip(ideal_point, nadir_point)
        ]
    ...
    def get_next_reference_point(self, *fronts):
        if self.iteration_count >= self.max_iterations - 1:
            ...
            normalized_front = self.normalize_front(
                self.composite_front, translated_front
            )
            assigned_vectors = self.assign_vectors(
                normalized_front, self.reference_vectors
            )
            max_assigned_vector = None
            if self.iteration_count < self.it_learning_phase:
                self.preference = self.generate_RP_learning(
                    ideal_point, translated_front, assigned_vectors
                )
            else:
                if self.iteration_count == self.it_learning_phase:
                    max_assigned_vector = self.get_max_assigned_vector(assigned_vectors)
