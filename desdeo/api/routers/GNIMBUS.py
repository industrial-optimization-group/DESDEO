...
class NIMBUSVoteResponse(NIMBUSBasicResponse):
    """The response from GNIMBUS vote endpoint."""

    type: str | None = Field(description="Type of the response (vote or None).", default=None)

class NIMBUSChooseResponse(NIMBUSBasicResponse):
    """The response from GNIMBUS vote endpoint."""

    chosen_solutions: dict[int, list[float]] = Field(description="Chosen solutions selected by the decision maker.")


class NIMBUSFullResponse(NIMBUSVoteResponse):
...
def init_nimbus(
    ...
    if not solutions:
        ...
        to_vote_solutions = {sol.id: sol.objectives for sol in solutions if sol.to_vote}
    ...
    return NIMBUSFullResponse(
        type="vote" if bool(to_vote_solutions) else None,
        ...
        current_solutions=to_vote_solutions if bool(to_vote_solutions) else {sol.id: sol.objectives for sol in solutions if sol.current},
...
async def solutionVote(
    ...
    user: Annotated[User, Depends(get_current_user)],
    db: Annotated[DB, Depends(database_dependency)],
) -> NIMBUSVoteResponse | FakeNIMBUSResponse:
    ...
    if not grequest.cached:
        ...
        solutions = (
            ...
            .filter((SolutionArchive.current==True) | (SolutionArchive.to_vote==True))
            .order_by(SolutionArchive.id.asc()))
        )

        to_vote_solutions = {sol.id: sol.objectives for sol in solutions if sol.to_vote}

        if bool(to_vote_solutions):
            return NIMBUSVoteResponse(
                type="vote",
                current_solutions=to_vote_solutions
            )
...
async def iterate(
    ...
    if not grequest.cached:
        ...
        reference_solution = list(reference_solutions.values())[0]
...
async def save(
    ...
    Returns:
        NIMBUSSaveResponse: The response from the GNIMBUS save algorithm.
    """
...
async def choose(
    ...
    user: Annotated[User, Depends(get_current_user)],
    db: Annotated[DB, Depends(database_dependency)],
) -> NIMBUSChooseResponse | NIMBUSVoteResponse | FakeNIMBUSResponse:
    ...
    return NIMBUSBasicResponse(
        current_solutions={sol.id: sol.objectives for sol in solutions},
