...
def check_solver(problem_db: ProblemDB):
    ...
    if metadata is not None:
        solver_metadata_list = [
            metadata
            for metadata in metadata.all_metadata
            if metadata.metadata_type == "solver_selection_metadata"
        ]
        if solver_metadata_list != []:
            solver_metadata = solver_metadata_list[-1]

    if solver_metadata is not None:
        solver = available_solvers[solver_metadata.solver_string_representation][
            "constructor"
        ]
    else:
        solver = None
...
def get_metadata(
    ...
    if not problem_db:
        raise HTTPException(
            status_code=404, detail=f"Problem with ID {request.problem_id} not found!"
        )
...
def select_solver(
    ...
    if request.solver_string_representation not in [
        x for x, _ in available_solvers.items()
    ]:
        raise HTTPException(
    ...
    problem_metadata = problem_db.problem_metadata or ProblemMetaDataDB(
        problem_id=problem_db.id, problem=problem_db
    )
...
def get_representative_solution_set(
    ...
    if repr_set is None:
        raise HTTPException(
            status_code=404, detail=f"Representative set with ID {set_id} not found."
        )
...
@router.delete(
    "/representative_solution_set/{set_id}", status_code=status.HTTP_204_NO_CONTENT
)
def delete_representative_solution_set(
    ...
    if repr_metadata is None:
        raise HTTPException(
            status_code=404,
            detail=f"Representative solution set with ID {set_id} not found.",
        )
...
def get_problem_json(
    ...
    return JSONResponse(
        content=json.loads(problem.model_dump_json()), status_code=status.HTTP_200_OK
    )
