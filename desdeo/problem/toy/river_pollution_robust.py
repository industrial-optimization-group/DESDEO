...
from desdeo.problem.porcelain import PorcelainProblem, Variable, Objective
...
class RiverPollutionRobust(PorcelainProblem):
    ...
    def robustness(bod_fishery, bod_city):
        ...
        #evaluating all discretized values
            wq_fishery_list.append(wq_fishery(fishery_interval[0] + per/num_com*i,
                                                   city_interval[0] + per/num_com*i))
            wq_city_list.append(wq_city(fishery_interval[0] + per/num_com*i,
                                                   city_interval[0] + per/num_com*i))
            fishery_roi_list.append(fishery_roi(fishery_interval[0] + per/num_com*i,
                                                   city_interval[0] + per/num_com*i))
            city_tax_list.append(city_tax(fishery_interval[0] + per/num_com*i,
                                                   city_interval[0] + per/num_com*i))
            
        #taking max and min for each
        upper = [max(wq_fishery_list), max(wq_city_list), max(fishery_roi_list), max(city_tax_list)]
        lower = [min(wq_fishery_list), min(wq_city_list), min(fishery_roi_list), min(city_tax_list)]
        diff = []
