...
def test_point_search():
    ...
    assert np.isclose(run1[0][1], run2[1][1]).all()  # pylint: disable=E1101
...
def test_classification(method):
    vals = method.init_iteration()
    method.problem.selected = vals[0][1]
    cls = NIMBUSClassification(method, vals[0][1])
    assert cls.reference_point() == vals[0][1]
