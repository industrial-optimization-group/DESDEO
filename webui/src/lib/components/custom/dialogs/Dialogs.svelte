<script lang="ts">
  ...
  import type { ConfirmDialogOptions, InputDialogOptions, HelpDialogOptions } from "./dialogs";
...
{#if current?.type === "help"}
  <HelpDialog
    open={true}
    {...(current.props as HelpDialogOptions)}
    onCancel={() => {
      (current.props as HelpDialogOptions)?.onCancel?.();
      closeDialog();
    }}
  />
{/if}
