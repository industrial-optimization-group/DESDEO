...
async function handle_init(body: any, refreshToken: string) {
	...
	if (response.error) {
		console.error(
			`GNIMBUS initialize API error: ${response.error.detail} (Status: ${response.response?.status})`
		);
		throw new Error(
			`GNIMBUS initialize API error: ${response.error.detail} (Status: ${response.response?.status})`
		);
...
async function get_all_iterations(body: any, refreshToken: string) {
	...
	if (
		...
		error.detail === 'Problem has not been initialized!'
	) {
		return {
			success: false,
	...
	if (response.error) {
		console.error(
			`GNIMBUS full_response API error: ${response.error.detail} (Status: ${response.response?.status})`
		);
		throw new Error(
			`GNIMBUS full_response API error: ${response.error.detail} (Status: ${response.response?.status})`
		);
...
async function switch_phase(body: any, refreshToken: string) {
	...
	if (response.error) {
		console.error(
			`GNIMBUS toggle_phase API error: ${response.error.detail} (Status: ${response.response?.status})`
		);
		throw new Error(
			`GNIMBUS toggle_phase API error: ${response.error.detail} (Status: ${response.response?.status})`
		);
...
async function handle_get_maps(body: any, refreshToken: string) {
	...
	try {
		...
		if (response.error) {
			...
			throw new Error(
				`utopia map API error: ${JSON.stringify(response.error)} (Status: ${response.response?.status})`
			);
		}
