<script lang="ts">
	...
	async function handle_finish() {
		...
		final_choice_state = true; // Set the final choice state to true
		console.log('Final choice confirmed, proceeding with solution review.');
	}
...
{:else}
	<BaseLayout showLeftSidebar={!!problem} showRightSidebar={true}>
		...
		{#snippet visualizationArea()}
			<div class="grid h-full w-full grid-cols-2 gap-6"></div>
		{/snippet}
