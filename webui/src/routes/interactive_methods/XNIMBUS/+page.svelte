<script lang="ts">
	...
	// Check if all objectives in a problem are twice differentiable
	export function areAllObjectivesTwiceDifferentiable(problem: ProblemInfo | null): boolean {
		...
		return problem.objectives.every(objective => 
			objective.is_twice_differentiable === true
		);
	}
</script>
...
{:else}
	<BaseLayout showLeftSidebar={!!problem} showRightSidebar={areAllObjectivesTwiceDifferentiable(problem)}>
		{#snippet leftSidebar()}
