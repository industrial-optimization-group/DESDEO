...
<script lang="ts">
	...
	const jsonFileForm = superForm(data.form, {
		taintedMessage: 'Are you sure you want to leave?',
		dataType: 'json',
		onError({ result }) {
			$message = result.error.message || 'Unknown error';
	...
	const {
		form: jsonFileFormData,
		message: jsonFileMessage,
		enhance: jsonFileEnhance,
		errors: jsonFileErrors
	} = jsonFileForm;
	...
	function addVariable(kind: 'scalar' | 'tensor') {
		if (kind === 'scalar') {
			...
			$formData.variables = [...$formData.variables, variable];
		} else {
	...
	function addConstant(kind: 'scalar' | 'tensor') {
		if (kind === 'scalar') {
			...
			$formData.constants = [...$formData.constants, constant];
		} else {
...
<section class="mx-10">
	...
	<div class="m-6 mx-auto max-w-4xl">
		...
		{#if $message}<h3 class="mb-4 text-red-500">{$message}</h3>{/if}
		{#if $errors._errors}<div class="mb-4 text-red-500">{$errors._errors}</div>{/if}
		...
		<form class="flex flex-col gap-4" method="POST" action="?/create" use:jsonFileEnhance>
			<Card class="p-6">
				<Name form={jsonFileForm} />
				<Description {form} />
			</Card>
			<FormButton>Add Problem</FormButton>
		</form>

		<h1 class="mt-10 text-center text-2xl font-semibold">Upload JSON file</h1>
